...
async function main() {
  ...
  const [cc, secondAccount] = await ethers.getSigners();
  console.log(`first account address is ${cc.address}`)
  console.log(`second account address is ${secondAccount.address}`)

  console.log("fetching the funds of the first account...")
  const fundsOfcc = await fundMe.listOfFunders(cc.address);
  console.log(`Current funds of first account is ${fundsOfcc}`)
